...
@push('scripts_top')
<script> gtag('event', 'conversion', { 'send_to': 'AW-795191608/sxGDCIf_29oZELjSlvsC', 'value':{{ $order->total_amount }}, 'currency': 'INR' }); 
</script>
@endpush
@section('content')
    ...
    @if(!empty($order) && $order->status === \App\Models\Order::$fail)
        <div class="no-result status-failed my-50 d-flex align-items-center justify-content-center flex-column">
            <div class="no-result-logo">
                <img src="{{ config('app.js_css_url') }}/assets/default/img/no-results/failed_pay.png" alt="">
            </div>
            <div class="d-flex align-items-center flex-column mt-30 text-center">
